...
#Change it to output the individual cryptocurrencies into their own files
...
# The CoinDesk 20
coindesk20_list = ['BTC', 'ETH', 'XMR', 'DOGE'] #'XRP', 'ADA', 'USDT', 'XLM', 'DOT', 'UNI', 'LINK', 'USDC', 'BCH', 'LTC', 'GRT', 'ETC', 'FIL', 'AAVE', 'ALGO', 'EOS'
raw_df = pd.DataFrame()
for coin in coindesk20_list:
    ...
    end_datetime = datetime(2020, 1, 1, 0, 0)
    datetime_checkpt = datetime(2021, 1, 1, 0, 0)
    ...
    coin_df['Symbol'] = coin
    coin_df = coin_df[['Datetime', 'Symbol', 'Open', 'High', 'Low', 'Close']].reset_index(drop=True)
    coin_df.to_csv(coin +'.csv', index=False)
    print("Finished Exporting: " + coin + " Data File")
